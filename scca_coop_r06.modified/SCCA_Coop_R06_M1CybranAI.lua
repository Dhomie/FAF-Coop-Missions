--****************************************************************************
--**
--**  File     : /maps/SCCA_Coop_R06.modified/SCCA_Coop_R06_M1CybranAI.lua
--**  Author(s): Dhomie42
--**
...
function M1CybranDebugBaseAirAttacks()
	local opai = nil
		...
		opai:SetChildActive('All', false)
		opai:SetChildrenActive({'AirSuperiority', 'StratBombers', 'CombatFighters', 'Gunships'})
		opai:SetChildCount(Difficulty + 1)
...
function M1CybranDebugBaseAirDefense()
	...
	local ChildType = {'AirSuperiority', 'Gunships', 'StratBombers', 'CombatFighters'}
	
	--Maintains [5, 10, 15] units defined in ChildType
	--Create platoons for each unit type.
	for k = 1, table.getn(ChildType) do
		opai = M1CybranDebugBase:AddOpAI('AirAttacks', 'M1_Cybran_Debug_AirDefense' .. ChildType[k], --Example: 'M1_Cybran_Debug_AirDefense_AirSuperiority'
			{
