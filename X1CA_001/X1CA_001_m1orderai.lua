--****************************************************************************
--**
--**  File     : /maps/X1CA_001/X1CA_001_m1orderai.lua
--**  Author(s): Jessica St. Croix
--**
--**  Summary  : Order army AI for Mission 1 - X1CA_001
--**
--**  Copyright © 2007 Gas Powered Games, Inc.  All rights reserved.
--****************************************************************************
local BaseManager = import('/maps/X1CA_001/X1CA_001_BaseManager.lua')--import('/lua/ai/opai/basemanager.lua')
...
function OrderM1WestBaseAI()
    ...
    OrderM1WestBase:StartNonZeroBase({4, 8, 12})
	OrderM1WestBase:SetMaximumConstructionEngineers(8)
...
function OrderM1WestBaseAirAttacks()
    ...
    -- sends 4, 8, 12 [gunships, combat fighter] if player has >= 8, 6, 4 T2/T3 AA towers
...
function OrderM1EastBaseAI()
    ...
    OrderM1EastBase:StartNonZeroBase({4, 8, 12})
...
function OrderM1EastBaseLandAttacks()
    ...
    for i = 1, Difficulty do
        opai = OrderM1EastBase:AddOpAI('BasicLandAttack', 'M1_LandDefense1_' .. i,
            {
                PlatoonData = {
                    PatrolChain = 'East_Beach_Patrol_Chain',					--East_Beach_Patrol_Chain = Original, M1_OrderToCybran_Chain = New
                },
    ...
    for i = 1, Difficulty do
        opai = OrderM1EastBase:AddOpAI('BasicLandAttack', 'M1_LandDefense2_' .. i,
            {
                PlatoonData = {
                    PatrolChain = 'West_Beach_Patrol_Chain',					--West_Beach_Patrol_Chain = Original
                },
                MasterPlatoonFunction = {SPAIFileName, 'PatrolThread'},
                Priority = 140,
            }
...
function M1DisableOrderBases()
    if(OrderM1WestBase) then
        OrderM1WestBase:SetBuild('Engineers', false)
        OrderM1WestBase:SetBuildAllStructures(false)
        OrderM1WestBase:SetActive('AirScouting', false)
        OrderM1WestBase:SetActive('LandScouting', false)
        OrderM1WestBase:BaseActive(false)
    end
	
	if(OrderM1EastBase) then
        OrderM1EastBase:SetBuild('Engineers', false)
        OrderM1EastBase:SetBuildAllStructures(false)
        OrderM1EastBase:SetActive('AirScouting', false)
        OrderM1EastBase:SetActive('LandScouting', false)
        OrderM1EastBase:BaseActive(false)
    end
end