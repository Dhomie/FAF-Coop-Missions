...
local M3UEFAI = import('/maps/X1CA_001/X1CA_001_m3uefai.lua')
...
local AIs = {Seraphim, Order, UEF, Civilians, Cybran, Aeon}
local EnemyAIs = {Seraphim, Order}
local AlliedAIs = {UEF, Civilians, Cybran, Aeon}
...
function OnPopulate(scenario)
	...
	-- -------------------
	-- Civilian South City Wreck, and South City intact, needed in Phase/Mission 1 due to the change of the Operation Area
	-- -------------------
...
function OnStart(self)
    ...
    ScenarioFramework.AddRestriction(Player1, categories.xaa0202) -- Aeon Mid Range fighter (Swift Wind)
    ScenarioFramework.AddRestriction(Player1, categories.xal0203) -- Aeon Assault Tank (Blaze)
    ...
    ScenarioFramework.AddRestriction(Player1, categories.xrl0305) -- Cybran Brick
    ...
    ScenarioFramework.AddRestriction(Player1, categories.xel0305) -- UEF Percival
	...
	ScenarioFramework.AddRestriction(Cybran, categories.url0105 + categories.url0208)
	ScenarioFramework.AddRestriction(Aeon, categories.ual0105 + categories.ual0208)
	...
	ForkThread(BuffAIEconomy, AlliedAIs, AlliedEcoRate)
end
...
function IntroNISPart1()
    ...
    if not SkipNIS1 then
        ...
        ScenarioFramework.CreateVisibleAreaLocation( 30, ScenarioUtils.MarkerToPosition( 'M1_NIS_Vis_1' ), 25, ArmyBrains[Player1] )
        ScenarioFramework.CreateVisibleAreaLocation( 30, ScenarioUtils.MarkerToPosition( 'M1_NIS_Vis_2' ), 25 + NIS1InitialDelay, ArmyBrains[Player1] )
        ...
        ScenarioFramework.CreateVisibleAreaLocation( 30, ScenarioUtils.MarkerToPosition( 'M1_NIS_Vis_4' ), 25 + NIS1InitialDelay, ArmyBrains[Player1] )
        ScenarioFramework.CreateVisibleAreaLocation( 30, ScenarioUtils.MarkerToPosition( 'M1_NIS_Vis_5' ), 25 + NIS1InitialDelay, ArmyBrains[Player1] )
        ...
        ScenarioFramework.CreateVisibleAreaLocation( 50, ScenarioUtils.MarkerToPosition( 'Order_M1_East_Base_Marker' ), 25 + NIS1InitialDelay, ArmyBrains[Player1] )
...
function IntroNISPart2()
    ...
    elseif(LeaderFaction == 'aeon') then
        ScenarioFramework.SetAeonAlly1Color(Player1)
    end

    if(LeaderFaction == 'cybran') then
        ScenarioInfo.PlayerCDR = ScenarioUtils.CreateArmyUnit('Player1', 'CybranPlayer')
    elseif(LeaderFaction == 'uef') then
        ScenarioInfo.PlayerCDR = ScenarioUtils.CreateArmyUnit('Player1', 'UEFPlayer')
    elseif(LeaderFaction == 'aeon') then
        ScenarioInfo.PlayerCDR = ScenarioUtils.CreateArmyUnit('Player1', 'AeonPlayer')
    end
...
function StartMission1()
	...
	if(LeaderFaction == 'aeon') then
			ScenarioInfo.PlayerCDR:CreateEnhancement('AdvancedEngineering')
			ScenarioInfo.PlayerCDR:CreateEnhancement('T3Engineering')
		...
		elseif(LeaderFaction == 'uef') then
			...
			ScenarioInfo.PlayerCDR:CreateEnhancement('T3Engineering')
			ScenarioInfo.PlayerCDR:CreateEnhancement('ResourceAllocation')
			ScenarioInfo.PlayerCDR:CreateEnhancement('Shield')
		end
	...
	CustomFunctions.GateInAmaliaButton()
end
...
function M1TechReveal()
    ...
    if(LeaderFaction == 'uef') then
        ...
        ScenarioFramework.RemoveRestriction(Player1, categories.xel0305)
    elseif(LeaderFaction == 'cybran') then
        ScenarioFramework.Dialogue(OpStrings.X01_M01_080)           --xrl0305 Bricks
        ScenarioFramework.RemoveRestriction(Player1, categories.xrl0305)
    elseif(LeaderFaction == 'aeon') then
        ScenarioFramework.Dialogue(OpStrings.X01_M01_032)           --xal0203 Blaze assault tank
        ScenarioFramework.RemoveRestriction(Player1, categories.xal0203)
    end
end
...
function IntroMission2()
    ForkThread(
		...
		--IssueMove({ ScenarioInfo.CybranCDR }, ScenarioUtils.MarkerToPosition('Order_M1_East_Base_Marker')) --> This will remain here so the code itself can be copied to elsewhere
            ...
            local num = table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.MOBILE * categories.AIR) - categories.TECH1, false))
            ...
            if(table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.AIR * categories.EXPERIMENTAL, false)) >= 1) then
            ...
            trigger = {45, 30, 15}
            num = table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.MOBILE * categories.LAND) - categories.CONSTRUCTION - categories.TECH1, false))
            if(num > trigger[Difficulty]) then
                num = num - trigger[Difficulty]
                num = math.ceil(num/15)
                if(num > 5) then
            ...
            if(table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.LAND * categories.EXPERIMENTAL, false)) >= 1) then
                local numGroups = {3, 4, 5}
                for i = 1, numGroups[Difficulty] do
                    units = ScenarioUtils.CreateArmyGroupAsPlatoon('Order', 'M2_Order_Adapt_Gunship', 'GrowthFormation')
                    for k, v in units:GetPlatoonUnits() do
...
function AeonM2TechReveal()
    ...
    ScenarioFramework.RemoveRestriction(Player1, categories.xaa0202)
end
...
function IntroMission3()
    ForkThread(
        function()
            ...
            -- Order CDR
			-- TODO: Add Commander Enhancements, especially shields, because even with the buffed defenses, T3 UEF Gunship spam just makes it too easy
            ScenarioInfo.OrderCDR = ScenarioUtils.CreateArmyUnit('Order', 'Order_ACU')
            ScenarioInfo.OrderCDR:SetCustomName(LOC '{i Gari}')
			ScenarioInfo.OrderCDR:CreateEnhancement('CrysalisBeam')
			ScenarioInfo.OrderCDR:CreateEnhancement('ShieldHeavy')
			ScenarioInfo.OrderCDR:CreateEnhancement('HeatSink')
			ScenarioInfo.OrderCDR:SetAutoOvercharge(true)
...
function M3CounterAttack()
    ...
    local extractors = ScenarioFramework.GetCatUnitsInArea(categories.MASSEXTRACTION, ScenarioUtils.AreaToRect('M3_IslandMass_South_Area'), ArmyBrains[Player1])
    local num = table.getn(extractors)
    local total = 0
    quantity = {2, 4, 6}
    if(num > 0) then
        if(table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.ALLUNITS - categories.WALL, false)) < 400) then
        ...
        for i = 1, num do
            ...
            ScenarioFramework.PlatoonPatrolChain(units, 'M3_Order_Counter_EastAttack_Chain')
            total = total + 1
        end
    end
    if(total < quantity[Difficulty]) then
        extractors = ScenarioFramework.GetCatUnitsInArea(categories.MASSEXTRACTION, ScenarioUtils.AreaToRect('M3_IslandMass_North_Area'), ArmyBrains[Player1])
    ...
    quantity = {2, 5, 7}
    local scus = ArmyBrains[Player1]:GetListOfUnits(categories.SUBCOMMANDER, false)
    num = table.getn(scus)
    if(num > 0) then
        if(num > quantity[Difficulty]) then
            num = quantity[Difficulty]
    ...
    quantity = {1, 3, 10}
    local shields = ArmyBrains[Player1]:GetListOfUnits(categories.SHIELD * categories.STRUCTURE, false)
    ...
    quantity = {1, 3, 5}
    local battleships = ArmyBrains[Player1]:GetListOfUnits(categories.BATTLESHIP, false)
    ...
    local exp = ArmyBrains[Player1]:GetListOfUnits(categories.ueb2401, false)
    num = table.getn(exp)
    if(num > 0) then
        if(num > quantity[Difficulty]) then
            num = quantity[Difficulty]
    ...
    if(total < quantity[Difficulty]) then
        exp = ArmyBrains[Player1]:GetListOfUnits(categories.TECH3 * categories.ARTILLERY * categories.STRUCTURE, false)
        num = table.getn(exp)
        if(num > 0) then
            if(num > quantity[Difficulty] - total) then
                num = quantity[Difficulty] - total
    ...
    if(total < quantity[Difficulty]) then
        exp = ArmyBrains[Player1]:GetListOfUnits(categories.NUKE * categories.STRUCTURE, false)
        num = table.getn(exp)
        if(num > 0) then
            if(num > quantity[Difficulty] - total) then
                num = quantity[Difficulty] - total
    ...
    exp = ArmyBrains[Player1]:GetListOfUnits(categories.uel0401, false)
    num = table.getn(exp)
    if(num > 0) then
        if(num > quantity[Difficulty]) then
            num = quantity[Difficulty]
    ...
    if(total < quantity[Difficulty]) then
        exp = ArmyBrains[Player1]:GetListOfUnits(categories.url0402, false)
        num = table.getn(exp)
        if(num > 0) then
            if(num > quantity[Difficulty] - total) then
                num = quantity[Difficulty] - total
    ...
    if(total < quantity[Difficulty]) then
        exp = ArmyBrains[Player1]:GetListOfUnits(categories.ual0401, false)
        num = table.getn(exp)
        if(num > 0) then
            if(num > quantity[Difficulty] - total) then
                num = quantity[Difficulty] - total
    ...
    exp = ArmyBrains[Player1]:GetListOfUnits(categories.uaa0310, false)
    num = table.getn(exp)
    if(num > 0) then
        if(num > quantity[Difficulty]) then
            num = quantity[Difficulty]
    ...
    if(total < quantity[Difficulty]) then
        exp = ArmyBrains[Player1]:GetListOfUnits(categories.ura0401, false)
        num = table.getn(exp)
        if(num > 0) then
            if(num > quantity[Difficulty] - total) then
                num = quantity[Difficulty] - total
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.STRUCTURE * categories.DEFENSE) - categories.TECH1, false))
    quantity = {2, 6, 10}
    if(num > 0) then
        num = math.ceil(num/2)
        if(num > quantity[Difficulty]) then
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.AIR * categories.MOBILE) - categories.TECH1, false))
    quantity = {60, 40, 20}
    trigger = {30, 20, 10}
    if(num > quantity[Difficulty]) then
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.LAND * categories.MOBILE) - categories.TECH1 - categories.CONSTRUCTION, false))
    quantity = {70, 50, 30}
    trigger = {40, 25, 15}
    if(num > quantity[Difficulty]) then
        num = math.ceil(num/trigger[Difficulty])
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.ALLUNITS - categories.WALL, false))
    ...
    quantity = {1, 3, 5}
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL * categories.FACTORY, false))
    if(num > 0) then
        if(num > quantity[Difficulty]) then
            num = quantity[Difficulty]
        end
    ...
    if(table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.NAVAL * categories.MOBILE) - categories.TECH1, false)) >= t2limits[Difficulty] or
       table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL * categories.MOBILE * categories.TECH3, false)) >= t3limits[Difficulty]) then
    ...
    quantity = {20, 10, 5}
    if(table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL, false)) > quantity[Difficulty]) then
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL * categories.MOBILE * categories.TECH1, false))
    quantity = {20, 15, 10}
    trigger = {1, 2, 3}
    if(num > quantity[Difficulty]) then
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL * categories.MOBILE * categories.TECH2, false))
    quantity = {14, 10, 6}
    trigger = {1, 2, 3}
    if(num > quantity[Difficulty]) then
        num = math.ceil(num/quantity[Difficulty])
    ...
    num = table.getn(ArmyBrains[Player1]:GetListOfUnits(categories.NAVAL * categories.MOBILE * categories.TECH3, false))
    quantity = {3, 2, 1}
    trigger = {1, 2, 3}
    if(num > quantity[Difficulty]) then
        num = math.ceil(num/quantity[Difficulty])
...
function StartMission3()
    ...
    if(LeaderFaction == 'uef') then
        ...
        ScenarioFramework.CreateTimerTrigger(NavalTownAssign, 60)
    end
...
function M3UEFSecondaryPart1()
    ...
    ScenarioFramework.CreateTimerTrigger(NavalTownAttack, 120)
end
...
function IntroMission4()
    ForkThread(
	    ...
	    --ForkThread(CheatEconomy, Seraphim)
			...
			ScenarioUtils.CreateArmyGroup('UEF', 'M4_UEF_Exp_Bases', true)
			ScenarioUtils.CreateArmyGroup('UEF', 'M4_Foward_Defensive_Lines', true)
            -- ---------
            -- M4 UEF AI
            -- ---------
			...
			M4UEFAI.UEFM4ExpansionOneAI()
			--M4UEFAI.UEFM4ExpansionTwoAI()
			...
			ScenarioInfo.SeraphimCDR:CreateEnhancement('DamageStabilizationAdvanced')
			ScenarioInfo.SeraphimCDR:CreateEnhancement('RateOfFire')
			ScenarioInfo.SeraphimCDR:SetAutoOvercharge(true)
            for i =1, 6 do
...
function M4CounterAttack()
    ...
    trigger = {60, 40, 20}
    num = table.getn(ScenarioFramework.GetCatUnitsInArea((categories.MOBILE * categories.LAND) - categories.TECH1, ScenarioUtils.AreaToRect('M3_North_Area'), ArmyBrains[Player1]))
    if(num > trigger[Difficulty]) then
    ...
    trigger = {80, 60, 40}
    num = table.getn(ScenarioFramework.GetCatUnitsInArea((categories.MOBILE * categories.AIR) - categories.TECH1, ScenarioUtils.AreaToRect('M3_North_Area'), ArmyBrains[Player1]))
    if(num > trigger[Difficulty]) then
        num = math.ceil(num/15)
        if(num > 6) then
            num = 6
    ...
    local exp = ScenarioFramework.GetCatUnitsInArea(categories.AIR * categories.EXPERIMENTAL, ScenarioUtils.AreaToRect('M3_North_Area'), ArmyBrains[Player1])
    num = table.getn(exp)
    if(num > 0) then
        if(num > 4) then
            num = 4
    ...
    exp = ScenarioFramework.GetCatUnitsInArea(categories.LAND * categories.EXPERIMENTAL, ScenarioUtils.AreaToRect('M3_North_Area'), ArmyBrains[Player1])
    num = table.getn(exp)
    if(num > 0) then
        if(num > 4) then
            num = 4
    ...
    exp = ArmyBrains[Player1]:GetListOfUnits(categories.ueb2401, false)
    num = table.getn(exp)
    if(num > 0) then
        if(num > 2) then
            num = 2
    ...
    trigger = {14, 12, 10}
    if(table.getn(ArmyBrains[Player1]:GetListOfUnits((categories.NAVAL * categories.MOBILE) - categories.TECH1, false)) > trigger[Difficulty]) then
    ...
    exp = ArmyBrains[Player1]:GetListOfUnits(categories.BATTLESHIP, false)
    num = table.getn(exp)
    if(num > 0) then
        if(num > 5) then
            num = 5
...
function M4NukeWarning()
    ...
    --UEF players when havent completed the m3 town obj get an emphatic warning. Those who have get the general warning
    elseif (LeaderFaction == 'uef') then
        if(ScenarioInfo.M3S1UEF.Active) then
...
function FindSatellite()
    local found = false
    while(not found) do
        local Satellite = ArmyBrains[UEF]:GetListOfUnits(categories.xea0002, false)
        if(table.getn(Satellite) >= 1) then
            found = true
			UEF_Satellite_Ping(Satellite)
        else
            WaitSeconds(15)
        end
    end
...
function UEF_Satellite_Ping(platoon)
    local Satellite = ArmyBrains[UEF]:GetListOfUnits(categories.xea0002, false)
	
	ScenarioFramework.Dialogue(OpStrings.X01_M03_150)

	ScenarioInfo.UEF_Satellite = Satellite[1]
    ScenarioInfo.UEF_Satellite_Ping = PingGroups.AddPingGroup(OpStrings.X01_M03_SUPPORT_010_010, 'xea0002', 'move', OpStrings.X01_M03_SUPPORT_010_020)
    ScenarioInfo.UEF_Satellite_Ping:AddCallback(UEF_Satellite_Ping_Activate)
	
	ScenarioFramework.CreateUnitDestroyedTrigger(SatelliteDead, ScenarioInfo.UEF_Satellite)
	
end

function UEF_Satellite_Ping_Activate(location)
    IssueStop({ScenarioInfo.UEF_Satellite})
    IssueClearCommands({ScenarioInfo.UEF_Satellite})
    IssueMove({ScenarioInfo.UEF_Satellite}, location)
end

function SatelliteDead()
    ScenarioInfo.UEF_Satellite_Ping:Destroy()
	--FindSatellite()
end
...
function SetupFletcherTauntTriggers()
    ...
    FletcherTM:AddUnitKilledTaunt('TAUNT25', ScenarioInfo.UnitNames[Seraphim]['Seraphim_CDR'])                  --"You freaks..." when a particular outer-ring building is destroyed
...
function OnCtrlF3()
	LOG('*DEBUG: Starting')
	CustomFunctions.AreaReclaimCleanUp('M4_Playable_Area')
end
