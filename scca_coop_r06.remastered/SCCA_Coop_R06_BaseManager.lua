------------------------------------------------------------------------------------------
---- File     :  /maps/SCCA_Coop_R06.remastered/SCCA_Coop_R06_BaseManager.lua
---- Summary  : Base Manage edit for the 6th Cybran Mission, Operation: Liberation
...
local RebuildStructuresTemplate = import('/maps/SCCA_Coop_R06.remastered/SCCA_Coop_R06_BuildingTemplates.lua').RebuildStructuresTemplate
...
local BuildingCounterDefaultValues = {
    ...
    -- Difficulty 2
    {
		...
		--We want the AI to rebuild indefinitely for this mission for all difficulties
        --[[Wall = 1,
        ...
        for _, upgradeName in upgradeTable do
            ...
            if bpUpgrade then
                if not unit:HasEnhancement(upgradeName) then
                    -- If we already have upgrade at that slot, remove it first
						--Dhomie42: Enhancements that have an upgraded version also get removed
							--for example: 'AdvancedEngineering' --> 'T3Engineering', in which case, once 'AdvancedEngineering' is completed, it gets removed
							--but 'T3Engineering' requires that enhancement first, so 'AdvancedEngineering' is issued to upgrade once again
							--thus the ACU is stuck in a limbo of constant upgrading
                    ...
                    -- Check for required upgrades
						--Dhomie42: Maybe this check should be the first *if* statement instead ?
            ...
            if self.Active then
                for k, v in self.UpgradeTable do
                    ...
                    if unit and not unit.Dead then
                        ...
                        -- Factories are not in Idle state when assisting other factories (so gotta use unit.UnitBeingBuilt to make sure they're not building anything),
                        -- so if the basemanager grabs the factory for assisting before this upgrade thread, then it would never get upgraded
							--Dhomie: Factory upgrading takes priority over finishing any units/platoons, so the check for unit.UnitBeingBuilt is not needed
