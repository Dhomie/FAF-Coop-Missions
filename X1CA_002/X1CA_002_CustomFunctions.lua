...
function GateInDimitriDialogue()
    ...
    dialogue.OnButtonPressed = function(self, info)
        ...
        if info.buttonID == 1 then
			...
			--Cybran ACU spawns in, set to unkillable for debug purposes
			...
			-- AI bases initialized, along with a starting eco buff
			CybranAI.CybranMainBaseAI()
			CybranAI.CybranAirExpansionAI()
			CybranAI.CybranNavalExpansionAI()
			--ForkThread(AllyStartingMassEco, Cybran)
			
			--Delete the ping option
            ScenarioInfo.GateInDimitriPing:Destroy()
        end
    end
end

function GateInJamesButton()
    -- Setup ping
    ScenarioInfo.GateInJamesPing = PingGroups.AddPingGroup(OpStrings.Allied_Commander_GateIn_Title, 'uel0001', 'attack', OpStrings.Allied_Commander_GateIn_Description)
    ScenarioInfo.GateInJamesPing:AddCallback(GateInJamesDialogue)
end

function GateInJamesDialogue()
    -- Create a comfirmation dialogue for creation of allied commander
    local dialogue = CreateDialogue(OpStrings.Allied_Commander_GateIn_Confirmation, {'<LOC _Yes>', '<LOC _No>'})
    dialogue.OnButtonPressed = function(self, info)
        dialogue:Destroy()
        if info.buttonID == 1 then
			...
			-- UEF ACU spawns in, set to unkillable for debug purposes
			ScenarioInfo.UEFCDR = ScenarioFramework.SpawnCommander('UEF', 'James_ACU', 'Warp', 'CDR James', false, false,
			{'AdvancedEngineering', 'T3Engineering', 'ResourceAllocation', 'Shield'})
			ScenarioInfo.UEFCDR:SetCanBeKilled(false)
			
			-- AI bases initialized, along with a starting eco buff
			UEFAI.UEFBaseAI()
			UEFAI.UEFAirExpansionAI()
			UEFAI.UEFNavalExpansionAI()
			--ForkThread(AllyStartingMassEco, UEF)
			--Delete the ping option
			ScenarioInfo.GateInJamesPing:Destroy()
			
        end
    end
end

-- Allied AI's starting mass economy
function AllyStartingMassEco(army)
	ArmyBrains[army]:GiveStorage('MASS', 75000)
	WaitSeconds(5)
    ArmyBrains[army]:GiveResource('MASS', 50000)
end

-- Allied AI's starting energy economy
function AllyStartingEnergyEco(army)
	ArmyBrains[army]:GiveStorage('ENERGY', 150000)
	WaitSeconds(5)
