...
local CustomFunctions = import('/maps/scca_coop_r05/scca_coop_r05_customfunctions.lua')
...
function NavalForceAI(self)
    ...
    while aiBrain:PlatoonExists(self) do
        ...
        if maxRange then maxRange = maxRange + 30 end
...
function AttackForceAI(self)
    ...
    while aiBrain:PlatoonExists(self) do
        ...
        -- if we're near our destination and we have a unit closeby to kill, kill it
        if table.getn(cmdQ) <= 1 and closestTarget and VDist3(closestTarget:GetPosition(), pos) < 30 and nearDest then
            self:StopAttack()
            self:AttackTarget(closestTarget)
            cmdQ = {1}
        -- if we have nothing to do, try finding something to do
        elseif table.empty(cmdQ) then
            self:StopAttack()
            cmdQ = AIPlatoonSquadAttackVector(aiBrain, self)
            stuckCount = 0
        -- if we've been stuck and unable to reach next marker? Ignore nearby stuff and pick another target
...
function AINavalPlanB(aiBrain, platoon)
	...
	if not navalAreas or table.empty(navalAreas) then
		return false
	end
...
---@param bRequired any
---@return boolean
function SendPlatoonWithTransportsNoCheck(aiBrain, platoon, destination, bRequired)

    AIAttackUtils.GetMostRestrictiveLayer(platoon)
    local data = platoon.PlatoonData
	local bSkipLastMove = false
    ...
    if platoon.MovementLayer == 'Land' or platoon.MovementLayer == 'Amphibious' then
        ...
        -- If we don't *need* transports, check if we enough transports for our platoon
        if not bRequired then
            --  If it doesn't work, bail, maybe the platoon will begin moving in the meantime
            if CustomFunctions.GetLoadTransports(platoon) == false then
                return false
            end
        else
            -- Transports are the only way to get where we want to go, if we can't load them, return false
		...
		-- If we didn't specify them, get markers around the target location, and pick the safest
			...
			for _, markerType in markerTypes do
				table.cat(landingPositions, AIUtils.AIGetMarkersAroundLocation(aiBrain, markerType, destination, 250))
			end
		end
		
		if not table.empty(landingPositions) then
		...
		while VDist2(PlatoonPosition[1], PlatoonPosition[3], transportLocation[1], transportLocation[3]) > 105 and not table.empty(platoon:GetSquadUnits('Scout')) do
			platoon:Stop()
	
			-- Transports get the 'Scout' role
			local threatMax = 15
			local transportNum = table.getn(platoon:GetSquadUnits('Scout')) or 1
			
			threatMax = transportNum * 15
